...
const submit = () => form.post(route('ships.store'));
</script>

<template>
    <Head :title="t('ships.add')" />
    <AppLayout>
        <template #header>{{ t('ships.add') }}</template>

        <div class="mb-3">
            <Link :href="route('ships.index')" class="text-decoration-none small fw-semibold">← {{ t('ships.back') }}</Link>
        </div>

        <PageHeader :title="t('ships.add')" :subtitle="t('ships.form_help')" />
        ...
        <form class="erp-card p-4 col-lg-8" @submit.prevent="submit">
            ...
            <div class="d-flex justify-content-end gap-2 mt-4">
                <Link :href="route('ships.index')" class="btn btn-erp-ghost">{{ t('common.cancel') }}</Link>
                <button class="btn btn-erp" :disabled="form.processing">
                    {{ form.processing ? t('common.saving') : t('ships.create') }}
                </button>
            </div>
        </form>
    </AppLayout>
