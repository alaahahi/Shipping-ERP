...
<template>
    ...
    <AppLayout>
        <template #header>{{ t('users.add') }}</template>
        ...
        <form class="erp-card p-4 col-lg-8 col-xl-6" @submit.prevent="submit">
            <div class="mb-4">
                <h2 class="h5 erp-display mb-1">{{ t('users.create_title') }}</h2>
                <p class="text-secondary small mb-0">{{ t('users.create_help') }}</p>
            </div>
            ...
            <div class="d-grid gap-3">
                ...
                <div>
                    ...
                    <input id="password" v-model="form.password" type="password" class="form-control form-erp-control" required autocomplete="new-password" />
                    <InputError :message="form.errors.password" />
                </div>
                <div>
                    <label for="password_confirmation" class="form-erp-label">{{ t('auth.confirm_password') }}</label>
                    <input id="password_confirmation" v-model="form.password_confirmation" type="password" class="form-control form-erp-control" required autocomplete="new-password" />
                </div>
            </div>
            ...
            <div class="d-flex justify-content-end gap-2 mt-4">
                ...
                <button type="submit" class="btn btn-erp" :disabled="form.processing">
                    {{ form.processing ? t('users.creating') : t('users.create_title') }}
                </button>
            </div>
        </form>
    </AppLayout>
